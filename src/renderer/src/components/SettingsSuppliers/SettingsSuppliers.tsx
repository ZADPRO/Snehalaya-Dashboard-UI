...
const SettingsSuppliers: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await axios.delete(`${import.meta.env.VITE_API_URL}/admin/suppliers/delete/${id}`, {
        headers: {
          Authorization: sessionStorage.getItem('token')
        }
      })
  ...
  return (
    <div className="card">
      ...
      >
        <Column header="S.No" body={(_rowData, options) => options.rowIndex + 1} style={{ minWidth: '1rem' }} />
