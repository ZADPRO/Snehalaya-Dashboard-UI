...
const SettingsAddNewBranch: React.FC<SettingsAddNewBranchProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = `${import.meta.env.VITE_API_URL}/admin/settings/branches`;
      const headers = { Authorization: sessionStorage.getItem('token') || '' };

      if (mode === 'add') {
        const response = await axios.post(url, branch, { headers });
        toast.current?.show({
          severity: 'success',
          summary: 'Branch Created',
          detail: response.data.message || 'Branch added successfully!',
          life: 3000,
        });
        onSave(response.data);
      } else {
        const response = await axios.put(url, branch, { headers });
        toast.current?.show({
          severity: 'success',
          summary: 'Branch Updated',
          detail: response.data.message || 'Branch updated successfully!',
          life: 3000,
        });
      ...
      // Handle duplicate entries (409 status code)
      if (error.response?.status === 409) {
        if (msg?.includes('code')) {
          toast.current?.show({
            severity: 'warn',
            summary: 'Duplicate Code',
            detail: 'Branch code already exists. Use a different code.',
            life: 4000,
          });
          ...
          toast.current?.show({
            ...
            detail: 'Branch name already exists. Use a different name.',
            life: 4000,
          });
        } else {
          toast.current?.show({
          ...
          <div className="flex-1">
            <FloatLabel className="flex-1 always-float">
              <InputNumber
                ...
                onValueChange={(e) => handleInputChange('refMobile', e.value?.toString() || '')}
                useGrouping={false}
